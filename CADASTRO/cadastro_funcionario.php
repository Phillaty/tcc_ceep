<!doctype html>  
<html lang="br"> 

<!--SE TU QUISER ESTUDAR OS CODIGO DE LAYOUT, VAI NO INDEX, LA EU COLOQUEI BASTANTE COMENTARIO-->
  <head>
    <?php include "../inclu/head.php";?>
    <?php include "../inclu/javascript.php"; ?>


<!--PARTE PARA A MASCARA QUE USEI NO TELEFONE E CPF
    <script>
      jQuery(function($){$("#Preco").mask("R$999,99");}, {reverse:true});
    </script>-->

    <script>
  function habilita(campo, idCampo) {
    document.getElementById(idCampo).disabled = campo.checked;
  }
</script>

  </head>
  <body id="pagina">

    <?php include "../inclu/menu.php"; ?>

    <div class="container-fluid">
      <div class="row">
        <nav class="navbar navbar-light b">
          <div class="col-2 ab"><img src="http://localhost/tcc/imagens/rt.png" class="img-fluid"></div>
          <form class="form-inline col-7 btt" method="post" action="../search_submit.php"  id="searchform">
            <input class="form-control mr-sm-2 col-8 offset-1 nya" type="search" placeholder="Pesquise por lanches aqui!!!" aria-label="Search" name="pesquisa" required>  
            <button class="btn h my-2 my-sm-0 col-2 wd" type="submit" name="submit">
              <i class="material-icons">search</i></button>
          </form>


          <div class="col-3">

                  <div class="alert-warning"><marquee>Você está na conta do admin :3</marquee></div>
              
          </div>
          </nav></div></div>




          <!--
          <div class="col-1 conta"><span class="badge badge-light">10</span></div>
          <div class="col-1 notificacao"><span class="badge badge-light">2</span></div>
          <div class="col-1 carrinho">
          <span class="badge badge-light">4</span></div>
          -->
        


<!--/////////////////////////////divisão entre a navbar e o menu com cardapio, sobre e tals/-->


<!--BEIBE BEIBE DO BIRULEIBE LEIBE?-->



      <div class="container-fluid">
      <div class="row">
      <div class="col-12 vvv">

<form class="col-5 png mt-5 card p-2" method="post" action="cadastro_funcionario.controller.php">
  <div class="text-center font-weight-bold"><h3>CADASTRO DE FUNCIONÁRIO</h3></div>
  <div class="dropdown-divider"></div>
  <div class="form-group">
    <label for="email_fu">Email</label>
    <input type="email" class="form-control" id="email_fu" name="email_fu" aria-describedby="emailHelp" placeholder="Insira email">
    <small id="emailHelp" class="form-text text-muted">We'll never share your email with anyone else.</small>
  </div>
  <div class="row">
  <div class="form-group col-6">
    <label>Senha</label>
    <input type="password" class="form-control" id="senha_fu" name="senha_fu" placeholder="Senha">
  </div>
    <div class="form-group col-6">
    <label>Confirmar senha</label>
    <input type="password" class="form-control" id="senha_fu2" name="senha_fu2" placeholder="Confirme a senha" oninput="validaSenha(this)">
  </div></div>
  <button type="submit" class="btn btn-primary">Cadastrar</button>
  <a class="bnt btn-warning text-center mt-3 p-2" href="../LOGIN/adm.php"><h6><b>Voltar</b></h6></a>
</form>
      <script>
            function validaSenha (input){ 
              if (input.value != document.getElementById('senha_fu').value) {
                input.setCustomValidity('Senhas diferentes!!');
                } else {
                input.setCustomValidity('');
                }
                }
      </script>





        
      </div>
      </div>
      </div>































      </div>


<!--FIM DO FORMULARIO-->



<!---///////////////////////////////////////////////////////////////////-->

    <script src="https://code.jquery.com/jquery-3.3.1.slim.min.js" integrity="sha384-q8i/X+965DzO0rT7abK41JStQIAqVgRVzpbzo5smXKp4YfRvH+8abtTE1Pi6jizo" crossorigin="anonymous"></script>
    <script src="https://cdnjs.cloudflare.com/ajax/libs/popper.js/1.14.3/umd/popper.min.js" integrity="sha384-ZMP7rVo3mIykV+2+9J3UJ46jBk0WLaUAdn689aCwoqbBJiSnjAK/l8WvCWPIPm49" crossorigin="anonymous"></script>
    <script src="https://stackpath.bootstrapcdn.com/bootstrap/4.1.1/js/bootstrap.min.js" integrity="sha384-smHYKdLADwkXOn1EmN1qk/HfnUcbVRZyYmZ4qpPea6sjB/pTJ0euyQp0Mk8ck+5T" crossorigin="anonymous"></script></div>
    

    <?php include "../inclu/foot.php"; ?>

    <script src="vendor/jquery/jquery.min.js"></script>
    <script src="vendor/bootstrap/js/bootstrap.bundle.min.js"></script>


  </body>
</html>